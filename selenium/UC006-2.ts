...
(async function testFilterByCity() {
    ...
    try {
        ...
        let filterOption = await driver.wait(until.elementLocated(By.xpath('//option[text()="Município"]')), 5000);
        ...
        // Find the new select element
        let newSelectElement = await driver.wait(until.elementLocated(By.css('select.block.w-44.text-sm.text-gray-500')), 5000);
        await driver.wait(until.elementIsVisible(newSelectElement), 2000);
        await newSelectElement.click();

        // Find and click the "São Paulo" option
        spOption = await newSelectElement.findElement(By.xpath('.//option[text()="São Paulo"]'));
        await driver.wait(until.elementIsVisible(spOption), 2000);
        await spOption.click();
