...
(async function testConnection() {
    ...
    try {
        ...
        let nameElement = await driver.wait(until.elementLocated(By.xpath('//div[contains(@class, "flex cursor-pointer")]/div/p[text()="Gabriel Samori Parreira"]')), 10000);
        await driver.wait(until.elementIsVisible(nameElement), 5000);
        await nameElement.click();

        // Ask for connection
        let buttonElement = await driver.wait(until.elementLocated(By.xpath('//button[contains(@class, "bg-blue-700") and text()="Solicitar amizade"]')), 10000);
        ...
        await driver.wait(until.elementIsVisible(emailField), 5000).sendKeys('s.parreira@aluno.ifsp.edu.br');
        ...
        buttonElement = await driver.wait(until.elementLocated(By.xpath('//button[contains(text(), "Amigo")]')), 10000);
        await driver.wait(until.elementIsVisible(buttonElement), 5000);
