...
(async function testEditProfile() {
    ...
    try {
        ...
        buttonXPath = '//button[@type="button" and contains(@class,"bg-cyan-700")]/span[text()="Confirmar Alterações"]';
        let confirmButton = await driver.wait(until.elementLocated(By.xpath(buttonXPath)), 10000);
        ...
        if (h2Text === "Gabriel S. Parreira") {
            console.log("The content of h2 is exactly 'Gabriel S. Parreira'.");
            console.log("UC013-1 test passed.");
        } else {
            console.log("The content of h2 is not 'Gabriel S. Parreira'.");
            console.log("UC013-1 test failed.");
        }
        // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        //Getting the data back to what it was for the sake of consistency and so that I can run the code again later

        // Define the XPath for the button within the specific div
        buttonXPath = '//div[@class="flex gap-5"]/button';

        // Locate the button using the specified XPath
        buttonElement = await driver.wait(until.elementLocated(By.xpath(buttonXPath)), 10000);
        ...
        // Change Name
        // 1. Locate the input field using its name attribute
        inputXPath = '//input[@name="nome"]';
        inputField = await driver.wait(until.elementLocated(By.xpath(inputXPath)), 10000);
        ...
        // 3. Clear the current value and send new keys to the input field
        await inputField.clear();
        await inputField.sendKeys('Gabriel Samori Parreira');

        // Add a description
        // 1. Locate the textarea using its name attribute
        textareaXPath = '//textarea[@name="sobre"]';
        textareaField = await driver.wait(until.elementLocated(By.xpath(textareaXPath)), 10000);
        ...
        // 3. Clear the current value (if any) and send new keys to the textarea
        await textareaField.clear();
        await textareaField.sendKeys('Descrição original.');

        // Change CEP
        // 1. Locate the input field for CEP inside the specific div using its name attribute
        cepInputXPath = '//div[contains(@class, "relative") and label[text()="CEP"]]/input[@name="cep"]';
        cepInputField = await driver.wait(until.elementLocated(By.xpath(cepInputXPath)), 10000);
        ...
        // 3. Clear the current value (if any) and send the CEP number to the input field
        await cepInputField.clear();
        await cepInputField.sendKeys('04634020');

        // Change estate street number
        // 1. Locate the input field for "numero" using its name attribute
        numeroXPath = '//div[label[text()="numero"]]/input[@name="numero"]';
        numeroInputField = await driver.wait(until.elementLocated(By.xpath(numeroXPath)), 10000);
        ...
        // 3. Clear the current value (if any) and send the desired number to the input field
        await numeroInputField.clear();
        await numeroInputField.sendKeys('253');

        // Change the addon (appartment or house No)
        // 1. Locate the input field for "complemento" using its name attribute
        complementoXPath = '//div[label[text()="complemento"]]/input[@name="complemento"]';
        complementoInputField = await driver.wait(until.elementLocated(By.xpath(complementoXPath)), 10000);
        ...
        // 3. Clear the current value (if any) and send the desired text to the input field
        await complementoInputField.clear();
        await complementoInputField.sendKeys('Apt. 152');

        // Confirm changes
        // 1. Locate the button with the specified text content
        buttonXPath = '//button[@type="button" and contains(@class,"bg-cyan-700")]/span[text()="Confirmar Alterações"]';
        confirmButton = await driver.wait(until.elementLocated(By.xpath(buttonXPath)), 10000);
