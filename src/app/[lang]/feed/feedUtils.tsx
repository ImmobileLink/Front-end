...
export async function deletePostAPI(postId: string, supabase: any) {
    const { error } = await supabase.from("publicacao").delete().eq("id", postId);
    if (error) {
        console.log(error)
        return false
    }
...
export async function publishPost({ idusuario, regiao, texto, imagem }: PostFormProps, supabase: any) {
...
export async function salvarPublicacaoAPI(savedItem: { idusuario: string, idpublicacao: string }, supabase: any) {
...
export async function removerPublicacaoSalvaAPI(userId: string, idpublicacao: string, supabase: any) {
    const { error } = await supabase
        .from("publicacaosalva")
        .delete()
        .eq("idusuario", userId)
...
export async function getPublicacoesSalvasPorEstadoAPI(userId: string | undefined, estado: string, supabase: any) {
...
export async function getPublicacoesSalvasPorEstadoCidadeAPI(userId: string | undefined, estado: string, cidade: string, supabase: any) {
    let {data, error} = await supabase
        .rpc("get_publicacoes_salvas", { idusuario: userId })
        .eq("regiao", JSON.stringify({
            cidade: cidade,
            estado: estado,
        }))
        .order("atualizadoem", { ascending: false })
