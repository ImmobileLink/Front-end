...
export default function BotaoAssocia({ link, userId }: botaoAddProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const { data } = await getEstadoBtnConexao(link?.id!, userId);
            if (data!.length > 0) {
                if (data![0].pendente) {
                    if (data![0].iniciativa == userId) {
                        setEstado("Pendente");
                    } else {
                        setEstado("Aceitar amizade");
                    }
    ...
    useEffect(() => {
        const fetchId = async () => {
            const { data } = await getIdConexao(link.id!, userId);
            if (data!.length > 0) {
    ...
    const handleClick = async () => {
        if (estado == "Solicitar amizade") {
            ...
            const { data, error } = await sendConvite(link?.id!, userId);
            if (error) {
                setEstado("Solicitar amizade");
            }
    ...
    const buttonClass = classNames("py-2 px-4 rounded", {
        ...
        "bg-yellow-700 hover:bg-yellow-800 dark:bg-yellow-600 dark:hover:bg-yellow-700 ":
            estado === "Pendente",
        "bg-blue-800 hover:bg-yellow-800 dark:bg-yellow-600 dark:hover:bg-yellow-700 ":
            estado === "Aceitar amizade",
    });
    ...
    return (
        <>
            <button
                onClick={handleClick}
                className={`min-w-[100px] h-[40px] text-white font-medium rounded-lg text-sm mb-1 mr-3 ${buttonClass}`}
            >
