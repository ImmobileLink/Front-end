    ...
    if (window.confirm(dict.sendsurvey)) {
      try {
        ...
        const requestBody = {
          ...
          visitDate: formataData(evento!.data_agendamento),
    ...
    <div className="z-50 absolute flex justify-center align-middle w-screen h-screen top-0 left-0 ">
      <div className="flex flex-col justify-between self-center w-10/12 md:w-8/12 lg:w-3/12 min-w-[400px] h-fit bg-white dark:bg-gray-900 rounded-2xl ring-1 ring-gray-300 dark:ring-gray-800 shadow-2xl">
        ...
        <div className="w-full flex justify-center gap-4 mb-4">
          <button className="px-8 py-2.5 bg-green-400 hover:bg-green-500 rounded-lg focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 " onClick={handleEnviaSurvey}><MdOutgoingMail className="h-5 w-5" /></button>
          <button className="px-8 py-2.5 bg-red-400 hover:bg-red-500 rounded-lg focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 " onClick={handleDeletaEvento}><LiaTrashAltSolid className="h-5 w-5" /></button>
        </div>
      </div>
    </div>
  );
